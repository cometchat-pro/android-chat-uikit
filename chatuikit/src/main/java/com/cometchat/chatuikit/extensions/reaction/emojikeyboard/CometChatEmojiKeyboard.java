...
public class CometChatEmojiKeyboard extends BottomSheetDialogFragment {
    ...
    public void setStyle(EmojiKeyboardStyle emojiKeyboardStyle) {
        if (emojiKeyboardStyle != null) {
            ...
            if (emojiAdapter != null) emojiAdapter.setStyle(emojiKeyboardStyle);
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        emojiListRecyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrollStateChanged(@NonNull RecyclerView recyclerView, int newState) {
                ...
                if (newState == RecyclerView.SCROLL_STATE_IDLE) isScrolling = false;
            ...
            public void onScrolled(@NonNull RecyclerView recyclerView, int dx, int dy) {
                ...
                ((Activity) getContext()).runOnUiThread(() -> tabLayout.getTabAt(linearLayoutManager.findLastVisibleItemPosition()).select());
        ...
        tabLayout.addOnTabSelectedListener(new TabLayout.OnTabSelectedListener() {
            ...
            public void onTabSelected(TabLayout.Tab tab) {
                if (!isScrolling) emojiListRecyclerView.scrollToPosition(tab.getPosition());
                if (selectedCategoryIconTint != 0)
                    tab.getIcon().setColorFilter(selectedCategoryIconTint, PorterDuff.Mode.SRC_IN);
                else
                    tab.getIcon().setColorFilter(getContext().getResources().getColor(R.color.primary), PorterDuff.Mode.SRC_IN);
            }

            @Override
            public void onTabUnselected(TabLayout.Tab tab) {
                if (categoryIconTint != 0)
                    tab.getIcon().setColorFilter(categoryIconTint, PorterDuff.Mode.SRC_IN);
                else
                    tab.getIcon().setColorFilter(getContext().getResources().getColor(R.color.accent600), PorterDuff.Mode.SRC_IN);
            }
