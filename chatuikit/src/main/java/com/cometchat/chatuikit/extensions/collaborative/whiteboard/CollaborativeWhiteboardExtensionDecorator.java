...
public class CollaborativeWhiteboardExtensionDecorator extends DataSourceDecorator {
    ...
    private String collaborativeWhiteBoardExtensionTypeConstant = ExtensionConstants.ExtensionType.WHITEBOARD;
    ...
    public List<CometChatMessageComposerAction> getAttachmentOptions(Context context, User user, Group group, HashMap<String, String> idMap) {
        if (!idMap.containsKey(UIKitConstants.MapId.PARENT_MESSAGE_ID)) {
            ...
            messageComposerActions.add(new CometChatMessageComposerAction().setId(ExtensionConstants.ExtensionType.WHITEBOARD).setTitle(context.getString(R.string.share_whiteboard)).setIcon(R.drawable.ic_collaborative_whiteboard).setTitleColor(theme.getPalette().getAccent()).setTitleAppearance(theme.getTypography().getSubtitle1()).setIconTintColor(theme.getPalette().getAccent700()).setBackground(theme.getPalette().getAccent100()).setOnClick(() -> {
                String id, type;
    ...
    public CometChatMessageTemplate getWhiteBoardTemplate() {
        return new CometChatMessageTemplate().setCategory(UIKitConstants.MessageCategory.CUSTOM).setType(collaborativeWhiteBoardExtensionTypeConstant).setOptions((context, baseMessage, isLeftAlign) -> ChatConfigurator.getUtils().getCommonOptions(context, baseMessage, isLeftAlign)).setBottomView((context, baseMessage, isLeftAlign) -> ChatConfigurator.getUtils().getBottomView(context, baseMessage, isLeftAlign)).setContentView((context, baseMessage, isLeftAlign) -> {
            if (baseMessage.getDeletedAt() == 0) {
                ...
                bubble.icon(context.getResources().getDrawable(R.drawable.ic_collaborative_whiteboard));
                ...
                bubble.setSubTitle("Open whiteboard to draw \ntogether.");
                bubble.setButtonText("Open Whiteboard");
                ...
                bubble.setBoardUrl(Extensions.getWhiteBoardUrl(baseMessage));
                return bubble;
    ...
    public String getLastMessage(Context context, BaseMessage lastMessage) {
        String message = null;
        if (UIKitConstants.MessageCategory.CUSTOM.equals(lastMessage.getCategory()) && ExtensionConstants.ExtensionType.WHITEBOARD.equalsIgnoreCase(lastMessage.getType()))
            message = Utils.getMessagePrefix(lastMessage, context) + context.getString(R.string.custom_message_whiteboard);
        return message;
    }
